...
/* Set the positionComp of a channel. (angle) is an integer from 0 to 360, that
 ...
 *
 * returns zero if error (no such channel or Mix_RegisterEffect() fails),
 *  nonzero if positionComp effect is enabled.
 *  Error messages can be retrieved from Mix_GetError().
 */
...
/* Set the current positionComp in the music stream.
   This returns 0 if successful, or -1 if it failed or isn't implemented.
   This function is only implemented for MOD music formats (set pattern
   order number) and for OGG, FLAC, MP3_MAD, MP3_MPG and MODPLUG music
   (set positionComp in seconds), at the moment.
*/
